...
def preprocess_parus(row, tokenizer, sep_token: str = '</s>'):
    sentence = tokenizer(row['question'] + " " +row['premise'], row['choice1'] + sep_token + row['choice2'])
    return sentence
...
def preprocess_dnqa(row, tokenizer):
    sentence = tokenizer(process_sentence(row['question']), process_sentence(row['passage']), max_length=512)
