...
def main(
    ...
    model = T5ForSequenceClassification.from_pretrained(model_dir, num_labels=3)
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    rcb_test_predict = np.argmax(trainer.predict(rcb_test).predictions[0], axis=1)
    ...
    prs_train = prs_raw_train.map(prepreprocess_parus).map(
        lambda x: preprocess_parus(x, tokenizer), remove_columns=cols_to_drop, batched=False
    )
    ...
    prs_test_predict = np.argmax(trainer.predict(prs_test).predictions[0], axis=1)
    prs_test_predict = [
    ...
    terra_test_pred = np.argmax(trainer.predict(terra_test).predictions[0], axis=1)
    ...
    lidirus_test_pred = np.argmax(trainer.predict(lidirus_test).predictions[0], axis=1)
    label_map_terra = {0: 'entailment' , 1: 'not_entailment'}
    lidirus_test_pred = [
    ...
    dnqa_test_pred = np.argmax(trainer.predict(dnqa_test).predictions[0], axis=1)
    ...
    rwsd_test_pred = np.argmax(trainer.predict(rwsd_test).predictions[0], axis=1)
