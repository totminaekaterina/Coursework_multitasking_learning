...
class FREDT5MultiTaskModel(torch.nn.Module):
    def __init__(self, model_params: dict) -> None:
        ...
        # --- CHANGED FOR STS ---
        self.sts_head = torch.nn.Linear(config.d_model * 2, 1).to(self.device)
...
class DataSet:
    ...
    def __init__(self):
        ...
        self.tasks["title"] = self.Task("title", "Дай название тексту: ", "gen", torch.nn.CrossEntropyLoss(ignore_index=-100))
        self.tasks["paraphrase"] = self.Task("paraphrase", "Перефразируй текст: ", "gen", torch.nn.CrossEntropyLoss(ignore_index=-100))
        self.tasks["sum"] = self.Task("sum", "Суммаризируй текст: ", "gen", torch.nn.CrossEntropyLoss(ignore_index=-100))
        self.tasks["qg"] = self.Task("qg", "Ответь на вопрос согласно тексту: ", "gen", torch.nn.CrossEntropyLoss(ignore_index=-100))
        self.tasks["qa"] = self.Task("qa", "Прочитай текст и ответь на вопрос: ", "gen", torch.nn.CrossEntropyLoss(ignore_index=-100))
...
class Trainer:
    ...
    def text_to_token(self, text: str, max_length=512) -> torch.Tensor:
        return (
            self.tokenizer(
                text,
            ...
            ).input_ids.squeeze(0)        #  [seq_len] → Tensor
        )

    def text_to_mask(self, text: str, max_length=512) -> torch.Tensor:
        return (
            self.tokenizer(
                text,
                max_length=max_length,
    ...
    def train_batch_gen(self, batch, label_name, loss_fn):
        # --- собираем входы из списка примеров ---
        texts  = [s["text"]        for s in batch]
        tgtstr = [s[label_name]    for s in batch]

        input_ids = torch.nn.utils.rnn.pad_sequence(
            [self.text_to_token(t, 512) for t in texts],
            batch_first=True,
            padding_value=self.tokenizer.pad_token_id,
        ).to(self.model.device)

        attn_mask = torch.nn.utils.rnn.pad_sequence(
            [self.text_to_mask(t, 512) for t in texts],
            batch_first=True,
            padding_value=0,
        ).to(self.model.device)

        label_ids = torch.nn.utils.rnn.pad_sequence(
            [self.text_to_token(t, 128) for t in tgtstr],
            batch_first=True,
            padding_value=self.tokenizer.pad_token_id,
        ).to(self.model.device)
    ...
    def train_batch_nli(self, batch, label_name, loss_fn):
        # входы
        input_ids = torch.nn.utils.rnn.pad_sequence(
            [self.text_to_token(s["text"], 512) for s in batch],
            batch_first=True,
            padding_value=self.tokenizer.pad_token_id,
        ).to(self.model.device)

        attn_mask = torch.nn.utils.rnn.pad_sequence(
            [self.text_to_mask(s["text"], 512) for s in batch],
            batch_first=True,
            padding_value=0,
        ).to(self.model.device)
        ...
        label_ids = torch.tensor(
            [label_map[s[label_name]] for s in batch],
    ...
    def train_batch_mc(self, batch, label_name, loss_fn):
        ...
        input_ids = torch.nn.utils.rnn.pad_sequence(
            [self.text_to_token(t) for t in texts],
            batch_first=True,
            padding_value=self.tokenizer.pad_token_id,
        ).to(self.model.device)

        attn_mask = torch.nn.utils.rnn.pad_sequence(
            [self.text_to_mask(t) for t in texts],
            batch_first=True,
            padding_value=0,
        ).to(self.model.device)
    ...
    def save_checkpoint(self, epoch, val_loss):
        checkpoint_path = self.output_dir + f"epoch_{epoch+1}.pth"
        torch.save({
            'epoch': epoch+1,
