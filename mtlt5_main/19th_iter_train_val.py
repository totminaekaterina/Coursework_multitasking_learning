...
class FREDT5MultiTaskModel(torch.nn.Module):
    ...
    def forward_nli(self, input_ids, attention_mask=None, decoder_input_ids=None):
        hidden_states = self.forward_first_encoder(input_ids, attention_mask)
        hidden_states = self.forward_second_encoder(hidden_states, attention_mask)  # did 2 steps before
    ...
    def forward_ner(self, input_ids, attention_mask=None, decoder_input_ids=None):
        hidden_states = self.forward_first_encoder(input_ids, attention_mask)
        # pooled = hidden_states.mean(dim=1)  # didn't use it
...
class Trainer:
    def __init__(self, model: FREDT5MultiTaskModel, dataset: DataSet, dataset_val: DataSet, tokenizer):
        ...
        self.batch_size = BATCH_SIZE_GPU
    ...
    def train(self, n_epoch):
        ...
        for epoch in range(n_epoch):
            global ENC_MAX, DEC_MAX
            loss = self.train_epoch(self.train_dataloader, True)
            loss_avg.append(loss)
            val_loss = self.train_epoch(self.train_dataloader_val, trainable=False)
            ...
            plt.legend()
            plt.grid(True)
            plt.savefig("/userspace/tev/cache/checkpoints/output_1bln/average_train_loss_3_4.png")
            plt.close()
            ...
            plt.legend()
            plt.grid(True)
            plt.savefig("/userspace/tev/cache/checkpoints/output_1bln/average_val_loss_3_4.png")
            plt.close()
            ...
            plt.legend()
            plt.grid(True)
            plt.savefig("/userspace/tev/cache/checkpoints/output_1bln/current_lr_hist_3_4.png")
            plt.close()
        ...
        while running_count > 0:
            for name in self.dataset.tasks:
                try:
                    ...
                    # оборачивание forward
                    with autocast(device_type="cuda", dtype=torch.bfloat16):
                        new_loss = self.train_batch(name, batch)

                    (new_loss / accumulation_steps).backward()
                ...
                step_count += 1
                if step_count % accumulation_steps == 0 and trainable:
    ...
    def train_batch_ner(self, batch, label_name, loss_fn):
        ...
        for sample in batch:
            ids, lbl = self.build_ner_labels(
                ...
                label_map=self.ner_label_map,  # заранее dict
            )
    ...
    def save_checkpoint(self, epoch: int, val_loss: float):
        checkpoint_path = os.path.join(
            self.output_dir, f"epoch_1bln_3_4_{epoch + 1}.pth"
        )
        torch.save(
            {
                "epoch": epoch + 1,
...
if __name__ == "__main__":
    ...
    train_data = process_multitask_dataset("/userspace/tev/cache/checkpoints/data_1bln/train_1.jsonl", 1)
    valid_data = process_multitask_dataset("/userspace/tev/cache/checkpoints/data_1bln/val.jsonl", 1)
    ...
    model_params = {
        "MODEL": "/userspace/tev/cache/checkpoints/model_1bln_2",  # или путь к FRED-T5-1.7B
        "LEARNING_RATE": 1e-4
    }
